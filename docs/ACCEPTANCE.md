...
- [ ] Multiple Spaces
- [ ] During a break, two- and multi-finger app gestures (scroll, swipe,
      magnify, rotate) are consumed while pointer movement and the visible
      press-and-hold skip control remain usable. A macOS system Space gesture
      must not expose underlying content; the overlay is reasserted on the
      destination Space.
